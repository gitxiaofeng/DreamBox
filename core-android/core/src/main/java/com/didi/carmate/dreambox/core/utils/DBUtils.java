...
public class DBUtils {
    ...
    public static int parseColor(DBBaseView dbBaseView, String color) {
        if (color.charAt(0) != '#' || (color.length() != 7 && color.length() != 9)) {
            StringBuilder errorMsg = new StringBuilder();
            if (dbBaseView.getId() != DBConstants.DEFAULT_ID_VIEW) {
                errorMsg.append("id: ").append(dbBaseView.getId()).append(",");
            }
            errorMsg.append(" type: ").append(dbBaseView.getClass().getSimpleName())
