...
+ (void)layoutAllViews:(DBViewModel *)model andView:(DBView *)view andRelativeViewPool:(DBRecyclePool *)pool
{
    ...
    //自适应,如果text，给定宽度，自适应高，则让系统计算，不做手算
    ...
    if(model.leftToLeft && model.rightToRight &&  !model.marginLeft && !model.marginRight){
        //强制布局，计算出左右参照视图的准确位置
        [view.superview setNeedsLayout];
        [view.superview layoutIfNeeded];
        UIView *leftRelationView = [pool getItemWithIdentifier:model.leftToLeft];
        UIView *rightRelationView = [pool getItemWithIdentifier:model.rightToRight];
        if (model.width != nil && [DBDefines db_getUnit:model.width] == 0) {
            ...
            if(leftRelationView != rightRelationView){
                CGFloat centerX = (CGRectGetMaxX(rightRelationView.frame) - CGRectGetMinX(leftRelationView.frame))/2;
                [view mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.centerX.mas_equalTo(leftRelationView.mas_left).offset(centerX);
                }];
            } else {
                [view mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.centerX.mas_equalTo(leftRelationView);
                }];
            }
        }
    }
